...
static GLfloat GetRequestedAnisotropy(void)
{
	switch (TextureAnisotropyLevel)
	{
	case 1:
		return 2.0f;
	case 2:
...
#define TA_MAXVERTICES 8192
#define TA_MAXTRIANGLES 8192
...
void InitOpenGL()
{
	AVP_RenderWorkers_Init();
	atexit(AVP_RenderWorkers_Shutdown);
	pglHint(GL_PERSPECTIVE_CORRECTION_HINT, GL_DONT_CARE);
	pglHint(GL_GENERATE_MIPMAP_HINT, GL_DONT_CARE);

#if GL_NV_multisample_filter_hint
	if (ogl_use_multisample_filter_hint)
	{
		pglHint(GL_MULTISAMPLE_FILTER_HINT_NV, GL_DONT_CARE);
	}
#endif
...
static void QueuePreparedRenderVertices(const RENDERVERTEX *src, int count,
										AVP_RenderVertexMode mode,
										const AVP_RenderVertexJobParams *params)
{
	AVP_GLVertex *dst = (AVP_GLVertex *)varrp;
	varrp += count;
	varrc += count;
	AVP_RenderWorkers_Submit(src, (size_t)count, dst, mode, params);
}

static AVP_RenderVertexJobParams MakeRenderJobParams(float z_near,
													 float recip_w,
													 float recip_h,
													 ...
													 int flags)
{
	...
	params.proj_x = (float)Global_VDB_Ptr->VDB_ProjX;
	params.proj_y = (float)Global_VDB_Ptr->VDB_ProjY;
	params.centre_x = (float)ScreenDescriptorBlock.SDB_CentreX;
	params.centre_y = (float)ScreenDescriptorBlock.SDB_CentreY;
	params.recip_w = recip_w;
	params.recip_h = recip_h;
	params.hud_z_offset = (float)HeadUpDisplayZOffset;
	params.color_r = r;
	params.color_g = g;
	params.color_b = b;
	params.color_a = a;
...
GLuint CreateOGLTexture(D3DTexture *tex, unsigned char *buf)
{
	...
	if (ogl_use_texture_filter_anisotropic && TextureAnisotropyLevel > 0)
	{
		pglGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &max_anisotropy);
		{
			GLfloat requested = GetRequestedAnisotropy();
			if (requested > max_anisotropy)
				requested = max_anisotropy;
			pglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, requested);
		}
	}
...
void D3D_ZBufferedGouraudTexturedPolygon_Output(POLYHEADER *inputPolyPtr, RENDERVERTEX *renderVerticesPtr)
{
...

	{
		...
		{
			if (renderVerticesPtr[i].SpecularR ||
				renderVerticesPtr[i].SpecularG ||
				renderVerticesPtr[i].SpecularB)
			{
				hasSpecular = 1;
				break;
			}
		...
		CheckTriangleBuffer(RenderPolygon.NumberOfVertices,
							hasSpecular ? RenderPolygon.NumberOfVertices : 0,
							0, 0, TextureHandle, RenderPolygon.TranslucencyMode, -1);
	}
	{
		AVP_RenderVertexJobParams params = MakeRenderJobParams(ZNear, RecipW, RecipH, 0, 0, 0, 0, 0);
		QueuePreparedRenderVertices(renderVerticesPtr, RenderPolygon.NumberOfVertices, AVP_RENDER_VERTEX_GOURAUD_TEXTURED, &params);
	}
}
...
void D3D_SkyPolygon_Output(POLYHEADER *inputPolyPtr, RENDERVERTEX *renderVerticesPtr)
{
	...
	{
		AVP_RenderVertexJobParams params = MakeRenderJobParams(0.0f, RecipW, RecipH, 0, 0, 0, 0, 0);
		params.hud_z_offset = 0.0f;
		QueuePreparedRenderVertices(renderVerticesPtr, RenderPolygon.NumberOfVertices, AVP_RENDER_VERTEX_SKY, &params);
	}
}
...
void D3D_ZBufferedCloakedPolygon_Output(POLYHEADER *inputPolyPtr, RENDERVERTEX *renderVerticesPtr)
{
	...
	{
		AVP_RenderVertexJobParams params = MakeRenderJobParams(ZNear, RecipW, RecipH, 0, 0, 0, 0, 0);
		QueuePreparedRenderVertices(renderVerticesPtr, RenderPolygon.NumberOfVertices, AVP_RENDER_VERTEX_CLOAKED, &params);
	}
}
...
void D3D_Decal_Output(DECAL *decalPtr, RENDERVERTEX *renderVerticesPtr)
{
	...
	{
		AVP_RenderVertexJobParams params = MakeRenderJobParams(ZNear, RecipW, RecipH, r, g, b, a, 0);
		QueuePreparedRenderVertices(renderVerticesPtr, RenderPolygon.NumberOfVertices, AVP_RENDER_VERTEX_DECAL, &params);
	}
}
...
void D3D_Particle_Output(PARTICLE *particlePtr, RENDERVERTEX *renderVerticesPtr)
{
	...
	CheckTriangleBuffer(RenderPolygon.NumberOfVertices, 0, 0, 0, TextureHandle, particleDescPtr->TranslucencyType, -1);
	{
		...
		if (particleDescPtr->IsDrawnInFront)
			zflags |= 1;
		if (particleDescPtr->IsDrawnAtBack)
			zflags |= 2;
		AVP_RenderVertexJobParams params = MakeRenderJobParams(ZNear, RecipW, RecipH, r, g, b, a, zflags);
		QueuePreparedRenderVertices(renderVerticesPtr, RenderPolygon.NumberOfVertices, AVP_RENDER_VERTEX_PARTICLE, &params);
	}
}
...
void D3D_PredatorThermalVisionPolygon_Output(POLYHEADER *inputPolyPtr, RENDERVERTEX *renderVerticesPtr)
{
	...
	{
		AVP_RenderVertexJobParams params = MakeRenderJobParams(ZNear, 0.0f, 0.0f, 0, 0, 0, 0, 0);
		QueuePreparedRenderVertices(renderVerticesPtr, RenderPolygon.NumberOfVertices, AVP_RENDER_VERTEX_THERMAL, &params);
	}
}
...
void D3D_ZBufferedGouraudPolygon_Output(POLYHEADER *inputPolyPtr, RENDERVERTEX *renderVerticesPtr)
{
	...
	{
		AVP_RenderVertexJobParams params = MakeRenderJobParams(ZNear, 0.0f, 0.0f, 0, 0, 0, 0, (flags & iflag_transparent) ? 4 : 0);
		QueuePreparedRenderVertices(renderVerticesPtr, RenderPolygon.NumberOfVertices, AVP_RENDER_VERTEX_GOURAUD, &params);
	}
}
...
static int InGameMenuTextScaleFixed(void);
static int InGameMenuTransformX(int x);
static int InGameMenuTransformY(int y);

void D3D_RenderMenuSmallString(char *stringPtr, int x, int y, int colour)
{
	struct VertexTag quadVertices[4];
	const int scale = InGameMenuTextScaleFixed();
	x = InGameMenuTransformX(x);
	y = InGameMenuTransformY(y);
	...
	while (*stringPtr)
	{
		unsigned char c = (unsigned char)*stringPtr++;
		...
		x += MUL_FIXED(scale, AAFontWidths[c]);
	}
}
...
{
	D3D_RenderHUDString_Centred(stringPtr, centreX, y, colour);
}
...
static int InGameMenuResolutionScaleFixed(void)
{
	if (ScreenDescriptorBlock.SDB_Height <= 0)
		return ONE_FIXED;
	return (int)(((long long)ScreenDescriptorBlock.SDB_Height * ONE_FIXED) / 480);
}

static int InGameMenuUserScaleFixed(void)
{
	switch (MenuTextScale)
	{
	case 1:
		return (5 * ONE_FIXED) / 4;
	case 2:
		return (3 * ONE_FIXED) / 2;
	default:
		return ONE_FIXED;
	}
}
...
static int InGameMenuTextScaleFixed(void)
{
	return MUL_FIXED(InGameMenuResolutionScaleFixed(), InGameMenuUserScaleFixed());
}

static int InGameMenuTransformX(int x)
{
	const int scale = InGameMenuResolutionScaleFixed();
	const int screenCentreX = ScreenDescriptorBlock.SDB_Width / 2;
	return screenCentreX + MUL_FIXED(x - screenCentreX, scale);
}

static int InGameMenuTransformY(int y)
{
	const int scale = InGameMenuResolutionScaleFixed();
	const int virtualMenuCentreY = 280;
	const int screenCentreY = ScreenDescriptorBlock.SDB_Height / 2;
	return screenCentreY + MUL_FIXED(y - virtualMenuCentreY, scale);
}

static int InGameMenuTextWidth(const char *textPtr)
{
	int width = 0;
...
int Hardware_RenderSmallMenuText(char *textPtr, int x, int y, int alpha, enum AVPMENUFORMAT_ID format)
{
	...
	case AVPMENUFORMAT_RIGHTJUSTIFIED:
		x -= MUL_FIXED(InGameMenuTextWidth(textPtr), InGameMenuUserScaleFixed());
		break;
	case AVPMENUFORMAT_CENTREJUSTIFIED:
		x -= MUL_FIXED(InGameMenuTextWidth(textPtr), InGameMenuUserScaleFixed()) / 2;
		break;
	}
	...
	return InGameMenuTransformX(x) + MUL_FIXED(InGameMenuTextWidth(textPtr), InGameMenuTextScaleFixed());
}
...
{
	D3D_Rectangle(
		InGameMenuTransformX(x1),
		InGameMenuTransformY(y1),
		InGameMenuTransformX(x2),
		InGameMenuTransformY(y2),
		r, g, b, 255);
}
...
void D3D_DrawSliderBar(int x, int y, int alpha)
{
	struct VertexTag quadVertices[4];
	const int scale = InGameMenuResolutionScaleFixed();
	const int sliderHeight = MUL_FIXED(11, scale);
	const int two = MUL_FIXED(2, scale);
	const int capOffset = MUL_FIXED(215, scale);
	const int endOffset = MUL_FIXED(213, scale);
	unsigned int colour = alpha >> 8;
	...
	CheckFilteringModeIsCorrect(FILTERING_BILINEAR_OFF);
	{
		...
		quadVertices[0].X = x;
		quadVertices[3].X = x;
		quadVertices[1].X = x + two;
		quadVertices[2].X = x + two;
	...
	}
	{
		...
		quadVertices[0].X = x + endOffset + two;
		quadVertices[3].X = x + endOffset + two;
		quadVertices[1].X = x + endOffset + two + two;
		quadVertices[2].X = x + endOffset + two + two;

		D3D_HUDQuad_Output(HUDFontsImageNumber, quadVertices, colour);
	}

	quadVertices[2].Y = y + MUL_FIXED(2, scale);
	quadVertices[3].Y = y + MUL_FIXED(2, scale);

	{
		...
		quadVertices[0].X = x + two;
		quadVertices[3].X = x + two;
		quadVertices[1].X = x + capOffset;
		quadVertices[2].X = x + capOffset;

		D3D_HUDQuad_Output(HUDFontsImageNumber, quadVertices, colour);
	}

	quadVertices[0].Y = y + MUL_FIXED(9, scale);
	quadVertices[1].Y = y + MUL_FIXED(9, scale);
	quadVertices[2].Y = y + sliderHeight;
	quadVertices[3].Y = y + sliderHeight;
...
void D3D_DrawSlider(int x, int y, int alpha)
{
	struct VertexTag quadVertices[4];
	const int scale = InGameMenuResolutionScaleFixed();
	const int sliderHeight = MUL_FIXED(5, scale);
	const int width = MUL_FIXED(9, scale);
	unsigned int colour = alpha >> 8;
	...
	CheckFilteringModeIsCorrect(FILTERING_BILINEAR_OFF);
	{
		...
		quadVertices[0].X = x;
		quadVertices[3].X = x;
		quadVertices[1].X = x + width;
		quadVertices[2].X = x + width;
...
void D3D_DrawParticle_Rain(PARTICLE *particlePtr, VECTORCH *prevPositionPtr)
{
	...
	if ((-vertices[0].vx <= vertices[0].vz) && (vertices[0].vx <= vertices[0].vz) && (-vertices[0].vy <= vertices[0].vz) && (vertices[0].vy <= vertices[0].vz))
	{
...
void PostLandscapeRendering()
{
	...
	if (!strcmp(LevelName, "fall") || !strcmp(LevelName, "fall_m"))
	{
		...
		while (numOfObjects)
		{
			...
			{
				if ((!strcmp(modulePtr->name, "fall01")) || (!strcmp(modulePtr->name, "well01")) || (!strcmp(modulePtr->name, "well02")) || (!strcmp(modulePtr->name, "well03")) || (!strcmp(modulePtr->name, "well04")) || (!strcmp(modulePtr->name, "well05")) || (!strcmp(modulePtr->name, "well06")) || (!strcmp(modulePtr->name, "well07")) || (!strcmp(modulePtr->name, "well08")) || (!strcmp(modulePtr->name, "well")))
				{
					drawWaterFall = 1;
				}
				else if ((!strcmp(modulePtr->name, "stream02")) || (!strcmp(modulePtr->name, "stream03")) || (!strcmp(modulePtr->name, "watergate")))
				{
					drawStream = 1;
				}
				else if ((!strcmp(modulePtr->name, "openwat03")) || (!strcmp(modulePtr->name, "openwat04")) || (!strcmp(modulePtr->name, "openwat04A")) || (!strcmp(modulePtr->name, "openwat02")))
				{
					drawStream2 = 1;
				}
			}
...
//			UpdateFMVTextures(1);
		...
		while (numOfObjects)
		{
			...
			if (modulePtr && modulePtr->name)
			{
				...
				else
				{
					...
					if ((!stricmp(modulePtr->name, "shaft01")) || (!stricmp(modulePtr->name, "shaft02")) || (!stricmp(modulePtr->name, "shaft03")) || (!stricmp(modulePtr->name, "shaft04")) || (!stricmp(modulePtr->name, "shaft05")) || (!stricmp(modulePtr->name, "shaft06")))
					{
	...
	else if (!stricmp(LevelName, "derelict"))
	{
		...
		while (numOfObjects)
		{
			...
			if (modulePtr && modulePtr->name)
			{
				if ((!stricmp(modulePtr->name, "start-en01")) || (!stricmp(modulePtr->name, "start")))
				{
	...
	else if (!stricmp(LevelName, "genshd1"))
	{
		while (numOfObjects)
		{
			...
			if (modulePtr && modulePtr->name)
			{
				if ((!stricmp(modulePtr->name, "largespace")) || (!stricmp(modulePtr->name, "proc13")) || (!stricmp(modulePtr->name, "trench01")) || (!stricmp(modulePtr->name, "trench02")) || (!stricmp(modulePtr->name, "trench03")) || (!stricmp(modulePtr->name, "trench04")) || (!stricmp(modulePtr->name, "trench05")) || (!stricmp(modulePtr->name, "trench06")) || (!stricmp(modulePtr->name, "trench07")) || (!stricmp(modulePtr->name, "trench08")) || (!stricmp(modulePtr->name, "trench09")))
				{
...
void D3D_DrawWaterPatch(int xOrigin, int yOrigin, int zOrigin)
{
	...
	for (x = 0; x < 16; x++)
	{
		...
		for (z = 0; z < 16; z++)
		{
			...
			{
				if (((-point->vx <= point->vz * 2) && (point->vx <= point->vz * 2) && (-point->vy <= point->vz * 2) && (point->vy <= point->vz * 2)))
				{
			...
			{
				if (((-point->vx <= point->vz) && (point->vx <= point->vz) && (-point->vy <= point->vz) && (point->vy <= point->vz)))
				{
					MeshVertexOutcode[i] = 1;
				}
				else
